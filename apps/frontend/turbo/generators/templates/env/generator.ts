...
const DOCS_URL = 'https://docs.dentia.app/environment-variables';
...
export function createEnvironmentVariablesGenerator(
  ...
  return plop.setGenerator('env', {
    ...
    prompts: [
      ...
      {
        type: 'input',
        name: 'values.DATABASE_URL',
        message: `What is the Aurora PostgreSQL connection string?\nFor more info: ${getUrlToDocs('DATABASE_URL')}\n`,
        default: allVariables.DATABASE_URL,
      },
      {
        type: 'input',
        name: 'values.COGNITO_REGION',
        message: `Which AWS region hosts Cognito (e.g. us-east-1)?\nFor more info: ${getUrlToDocs('COGNITO_REGION')}\n`,
        default: allVariables.COGNITO_REGION,
      },
      {
        type: 'input',
        name: 'values.COGNITO_USER_POOL_ID',
        message: `What is the Cognito User Pool ID?\nFor more info: ${getUrlToDocs('COGNITO_USER_POOL_ID')}\n`,
        default: allVariables.COGNITO_USER_POOL_ID,
      },
      {
        type: 'input',
        name: 'values.COGNITO_WEB_CLIENT_ID',
        message: `What is the Cognito Web App Client ID?\nFor more info: ${getUrlToDocs('COGNITO_WEB_CLIENT_ID')}\n`,
        default: allVariables.COGNITO_WEB_CLIENT_ID,
      },
      {
        type: 'password',
        name: 'values.COGNITO_WEB_CLIENT_SECRET',
        message: `What is the Cognito Web App Client Secret (leave blank if not used)?\nFor more info: ${getUrlToDocs('COGNITO_WEB_CLIENT_SECRET')}\n`,
        default: allVariables.COGNITO_WEB_CLIENT_SECRET,
      },
      {
        type: 'input',
        name: 'values.AWS_REGION',
        message: `Which AWS region hosts the application infrastructure?\nFor more info: ${getUrlToDocs('AWS_REGION')}\n`,
        default: allVariables.AWS_REGION,
      },
      {
        type: 'input',
        name: 'values.S3_BUCKET_NAME',
        message: `What S3 bucket stores uploaded assets?\nFor more info: ${getUrlToDocs('S3_BUCKET_NAME')}\n`,
        default: allVariables.S3_BUCKET_NAME,
      },
      {
        type: 'input',
        name: 'values.S3_PUBLIC_BASE_URL',
        message: `Optional: public base URL or CDN for serving S3 assets.\nFor more info: ${getUrlToDocs('S3_PUBLIC_BASE_URL')}\n`,
        default: allVariables.S3_PUBLIC_BASE_URL,
      },
      {
        type: 'input',
        name: 'values.APP_BASE_URL',
        message: `What is the primary public URL for the app (https://app.example.com)?\nFor more info: ${getUrlToDocs('APP_BASE_URL')}\n`,
        default: allVariables.APP_BASE_URL,
      },
      {
        type: 'list',
        name: 'values.NEXT_PUBLIC_BILLING_PROVIDER',
        message: `Which billing provider do you want to use?\nFor more info: ${getUrlToDocs('NEXT_PUBLIC_BILLING_PROVIDER')}\n`,
        choices: ['stripe'],
        default: 'stripe',
      },
      {
