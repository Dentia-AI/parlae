  ...
  implements BillingStrategyProviderService
{
  ...
  async queryUsage(params: z.infer<typeof QueryBillingUsageSchema>) {
    ...
    try {
      ...
      const value = summaries.data.reduce(
        (
          acc: number,
          summary: Stripe.Billing.MeterEventSummary,
        ): number => {
          return acc + Number(summary.aggregated_value ?? 0);
        },
        0,
      );
  ...
  async getSubscription(subscriptionId: string) {
    ...
    try {
      ...
      const lineItems = subscription.items.data.map(
