...
export function OutboundCallLogDetail({ callId }: { callId: string }) {
  const router = useRouter();
  const [call, setCall] = useState<OutboundCallDetail | null>(null);
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`/api/outbound/call-logs/${callId}`);
        if (!response.ok) {
          if (response.status === 404) setError('Outbound call record not found.');
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">Loading outbound call record...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !call) {
    return (
      <div className="text-center py-20">
        ...
        <Button variant="outline" className="mt-4" onClick={() => router.push('/home/outbound/call-logs')}>
          <ArrowLeft className="h-4 w-4 mr-2" />
          Back to Outbound Call Logs
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between">
        <div>
          <Button variant="ghost" size="sm" onClick={() => router.push('/home/outbound/call-logs')} className="mb-2 -ml-2">
          ...
          <CardContent className="p-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="font-semibold text-sm capitalize">{call.direction}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <CardContent className="p-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="font-semibold text-sm truncate">{call.campaignName}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent>
                <ul className="space-y-1.5">
                  {sd.actionsPerformed.map((action: string, i: number) => (
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-2">
              <DetailRow label="Name" value={call.contactName || sd?.patient_name} />
              <DetailRow label="Phone" value={call.phoneNumber} />
              <DetailRow label="Email" value={call.contactEmail || sd?.patient_email} />
              {sd?.patient_id && <DetailRow label="Patient ID" value={sd.patient_id} />}
            </CardContent>
          </Card>

          {(call.appointmentSet || sd?.appointment_booked) && (
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-2">
              ...
              <CardContent className="space-y-2">
                {(call.appointmentSet || sd?.appointment_booked) && (
                  <Badge className="bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300">Booked</Badge>
                )}
                <DetailRow label="Type" value={sd?.appointment_type} />
                <DetailRow label="Date" value={sd?.appointment_date} />
                <DetailRow label="Time" value={sd?.appointment_time} />
                <DetailRow label="Provider" value={sd?.provider_name} />
              </CardContent>
            </Card>
          )}
          ...
          {(call.insuranceVerified || call.paymentPlanDiscussed) && (
            <Card>
              ...
              <CardContent className="space-y-2">
                ...
                <DetailRow label="Provider" value={call.insuranceProvider} />
          ...
          {call.followUpRequired && (
            <Card className="border-orange-200 dark:border-orange-900/50">
              ...
              <CardContent>
                {sd?.follow_up_notes && (
                  <p className="text-sm">{sd.follow_up_notes}</p>
                )}
              </CardContent>
            </Card>
          )}
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              <DetailRow label="Type" value={callTypeLabels[call.callType] || call.callType} />
              ...
              {call.campaignName && <DetailRow label="Campaign" value={call.campaignName} />}
              {call.metadata?.disconnectionReason && (
                <DetailRow label="Disconnect" value={call.metadata.disconnectionReason} />
              )}
            </CardContent>
          </Card>
        </div>
