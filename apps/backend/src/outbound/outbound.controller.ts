...
export class OutboundController {
  ...
  async testRun(
    ...
    // Pull real contact data from an auto-generated (scanner) campaign, not a previous test run
    const existingCampaign = await this.prisma.outboundCampaign.findFirst({
      where: { accountId, callType, isAutoGenerated: true },
    ...
    this.logger.log({
      accountId,
      callType,
      existingCampaignId: existingCampaign?.id ?? null,
      realContactsFound: realContacts.length,
      firstContactSample: realContacts[0]
        ? { patientId: realContacts[0].patientId, hasCallContext: !!realContacts[0].callContext }
        : null,
      msg: '[Admin] Test run: resolved real contacts from existing campaign',
    });
