...
export class SikkaWritebackService {
  ...
  private async getWritebacksReadyForCheck(): Promise<Array<any>> {
    ...
    const allPending = await this.prisma.pmsWriteback.findMany({
      where: {
        result: 'pending',
    ...
    const readyForCheck: typeof allPending = [];
  ...
  async pollPendingWritebacks(): Promise<{ 
    ...
    const totalPending = await this.prisma.pmsWriteback.count({
  ...
  async getWritebackStatsByPractice(pmsIntegrationId: string) {
    const [total, pending, completed, failed, avgDuration] = await Promise.all([
      this.prisma.pmsWriteback.count({ 
      ...
      this.prisma.$queryRaw<Array<{ avg_duration: number }>>`
  ...
  async markStuckWritebacksAsFailed(): Promise<number> {
    ...
    const result = await this.prisma.pmsWriteback.updateMany({
      where: {
        result: 'pending',
 ...
 */
export async function pollSikkaWritebacks(prismaService: any) {
  const service = new SikkaWritebackService(prismaService);
 ...
 * Note: Use NestJS dependency injection instead when possible
 */
export async function retrySikkaWritebacks(prismaService: any) {
  const service = new SikkaWritebackService(prismaService);
